...
rel[Language, loc, M3] javaM3(loc project, ProjectDelta delta, map[loc repos,loc folders] checkouts, map[loc,loc] scratch) {  
  ...
  try {
    ...
    for (repo <- checkouts) {
      sources = getSourceRoots({checkouts[repo]});
      setEnvironmentOptions({*classpaths[checkouts[repo]]}, sources);
    
      result += {<java(), f, createM3FromFile(f)> | f <- find(checkouts[repo], "java")};
    }
  }
  catch "not-maven": {
...
rel[Language, loc, AST] javaAST(loc project, ProjectDelta delta, map[loc repos,loc folders] checkouts, map[loc,loc] scratch) {
  ...
  try {
    ...
    for (repo <- checkouts) {
      ...
      result += {<java(), f, declaration(createAstFromFile(f, true))> | f <- find(checkouts[repo], "java")};
    }
  }
  catch "not-maven": {
    jars = findJars(checkouts.folders);
